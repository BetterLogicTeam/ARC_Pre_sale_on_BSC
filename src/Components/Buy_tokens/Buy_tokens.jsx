...
function Buy_tokens(props, connect) {
  ...
  const buyARC = async () => {
    try {
      ...
      await USDC_ContractOf.methods.approve(ico_contract, value).send({
        from: acc,
      });
      ...
      let res = await axios.post("https://ico.archiecoin.online/send_token", {
        toaddress: acc,
        amount: web3.utils.toWei(GetEthValue),
      });
  ...
  const convertToUSDT = async () => {
    try {
      ...
      toast.success("Approved Successfully! 🎉");
      await ICO_ContractOf.methods.BuyARCWithBUSD(value).send({
        from: acc,
      });
      toast.success("Amount Deposited! 🎉");
      let res = await axios.post("https://ico.archiecoin.online/send_token", {
